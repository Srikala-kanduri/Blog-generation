...
GEMINI_API_URL = get_secret("GEMINI_API_URL") or "https://generativelanguage.googleapis.com/v1beta2/models"

OLLAMA_URL = get_secret("OLLAMA_URL") or "http://127.0.0.1:11434"
OLLAMA_MODEL = get_secret("OLLAMA_MODEL") or "qwen2.5:3b"
...
def _extract_groq_text(response):
    ...
    if isinstance(response, dict):
        ...
        if choices:
            ...
            if delta:
                return (delta.get("content") if isinstance(delta, dict) else getattr(delta, "content", "")) or ""

    return ""


def generate_with_gemini(prompt, max_tokens=6000):
    if not GEMINI_API_KEY:
        raise RuntimeError("Gemini API key is not configured.")

    url = f"{GEMINI_API_URL.rstrip('/')}/{GEMINI_MODEL}:generate"
...
def generate_with_groq(prompt, max_tokens=6000):
    if not GROQ_API_KEY:
...
def generate_with_ollama(prompt, max_tokens=6000):
    if not OLLAMA_URL:
        raise RuntimeError("OLLAMA_URL is not configured.")

    url = f"{OLLAMA_URL.rstrip('/')}/v1/generate"
    body = {
        "model": OLLAMA_MODEL,
        "prompt": prompt,
        "max_tokens": max_tokens,
    ...
    if isinstance(data, dict):
        ...
        if results:
            ...
            if isinstance(first, dict):
                output = first.get("output") or first.get("text")
                if isinstance(output, str):
                    return output.strip()
...
def validate_blog_output(text):
    ...
    if re.search(r"\b(lorem ipsum|placeholder text|dummy text|example text|test content)\b", cleaned, flags=re.I):
        return False

    if not re.search(r"^#\s+.+|^##\s+.+", cleaned, flags=re.M):
        return False

    return True


def generate_blog_content(prompt, max_tokens=6000):
    providers = [
        ("Gemini Flash 2.5 Pro", generate_with_gemini),
        ("Groq Llama 3.3", generate_with_groq),
    ...
    for provider_name, provider_fn in providers:
        try:
            output = provider_fn(prompt, max_tokens=max_tokens)
            if output and validate_blog_output(output):
                return output, provider_name
            errors.append(f"{provider_name} returned invalid output")
...
def get_image_url(query, used_urls=None):
    ...
    # Try multiple random pages for variety
    pages_to_try = [random.randint(1, 8) for _ in range(4)]
    ...
    # If no new images found, still return something from available pool
    all_images = get_image_urls(query, page=1)
    if all_images:
...
def optimize_image_query(prompt):
    """Generate highly specific image search queries to avoid ambiguous results."""
    prompt = re.sub(r"\b(?:diagram|placeholder|image of|illustration about|showing|displaying)\b", "", prompt, flags=re.IGNORECASE)
    prompt = re.sub(r"\s+", " ", prompt).strip(" ,.-")
    if not prompt:
        return "software development team coding computer"

    normalized = prompt.lower()

    # Specific tech mappings to avoid ambiguity
    tech_mappings = {
        r"\b(?:application logging|log management|logging system|system log)": "software application logging digital dashboard computer screen",
        r"\b(?:error handling|exception handling|runtime error|error message)": "software error message computer screen debugging code",
        r"\b(?:database|sql|nosql|mongodb|postgres)": "database server computer technology digital",
        r"\b(?:api|rest api|api gateway|webhook)": "software api integration computer code digital",
        r"\b(?:cloud computing|cloud services|aws|azure|gcp)": "cloud infrastructure servers technology data center",
        r"\b(?:machine learning|deep learning|neural network|ai model)": "artificial intelligence machine learning technology computer",
        r"\b(?:cybersecurity|security|encryption|firewall|threat)": "cybersecurity digital protection technology computer",
        r"\b(?:devops|ci/cd|deployment|docker|kubernetes)": "devops deployment container orchestration technology",
        r"\b(?:frontend|backend|ui|ux|interface design)": "software development web design user interface",
        r"\b(?:web development|web design|website|html|css|javascript)": "web development coding computer technology",
        r"\b(?:mobile app|mobile development|ios|android)": "mobile app development smartphone technology",
        r"\b(?:data science|analytics|big data|data visualization)": "data science analytics business intelligence",
        r"\b(?:blockchain|crypto|bitcoin|ethereum)": "blockchain cryptocurrency technology digital",
        r"\b(?:iot|internet of things|sensors|edge computing)": "iot sensors internet of things technology",
        r"\b(?:monitoring|observability|metrics|prometheus|grafana)": "software monitoring observability dashboard metrics",
        r"\b(?:testing|qa|automation|unit test)": "software testing quality assurance automation code",
    }
    ...
    if re.search(r"\b(?:business|startup|strategy|growth|sales|marketing|ecommerce)", normalized):
        return f"{prompt} business professional modern workplace"

    if re.search(r"\b(?:healthcare|medical|hospital|doctor|patient|biotech|wellness|pharma)", normalized):
        return f"{prompt} healthcare medical professional"

    if re.search(r"\b(?:education|learning|students|school|training|university|course)", normalized):
        return f"{prompt} education classroom learning modern"

    if re.search(r"\b(?:finance|banking|investment|cryptocurrency|trading|accounting)", normalized):
        return f"{prompt} finance banking professional technology"

    if re.search(r"\b(?:design|graphic|ux|ui|creative|branding)", normalized):
        return f"{prompt} design creative professional modern"

    # Default: add tech context
    return f"{prompt} technology software digital professional high quality"
...
def section_image_prompt(heading, section):
    ...
    return f"high quality editorial photography of {query_subject}, realistic, professional composition"
...
"""
    ...
    with st.spinner(f"Generating ~{word_count} word SEO blog..."):
        ...
        try:
            ...
            blog_body, provider_used = generate_blog_content(prompt, max_tokens=6000)
            stream_box.markdown(
                    f'<div class="card"><div class="card-title"> Generated Blog Preview</div>'
                    f'<div class="card-underline"></div>'
                    f'<div class="blog-output">{blog_preview_html(blog_body)}</div>'
                    f'<div class="provider-notice" style="margin-top:12px;font-size:0.9rem;color:#6b7280;">Generated with: {html.escape(provider_used)}</div></div>',
                    unsafe_allow_html=True
                )
            auto_meta = ""

            if generate_meta:
                meta_match = re.search(r"META:\s*(.+)", blog_body)
                if meta_match:
