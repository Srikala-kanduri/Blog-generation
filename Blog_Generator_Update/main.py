...
def get_image_url(query, used_urls=None):
    ...
    return random.choice(available_urls or image_urls)
...
def optimize_image_query(prompt):
    ...
    return f"{prompt}, high quality technical concept image"
...
def section_image_prompt(heading, section):
    ...
    if not topic_terms:
        topic_terms = f"technical illustration for {heading}"
    return (
        f"high quality technical illustration about {heading}; "
        f"visual context: {topic_terms}; modern digital interface style"
    )
