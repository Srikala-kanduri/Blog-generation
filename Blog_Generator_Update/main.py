...
def optimize_image_query(prompt):
    ...
    return f"{prompt} realistic editorial photography"
...
def section_image_prompt(heading, section):
    ...
    if not topic_terms:
        topic_terms = f"visual subject for {heading}"
    return (
        f"realistic editorial landscape image about {heading}; "
        f"visual context: {topic_terms}"
    )
