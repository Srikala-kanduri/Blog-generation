...
def optimize_image_query(prompt):
    ...
    if not prompt:
        return "high quality editorial photography, modern technical concept image"
    ...
    if re.search(
        ...
        return f"high quality editorial photography of {prompt}, computer interface screenshot, terminal window, high resolution"

    if re.search(r"\b(?:diagram|flowchart|chart|graph|infographic|mockup|wireframe|architecture|architecture diagram)\b", normalized, flags=re.IGNORECASE):
        return f"high quality infographic style image of {prompt}, clean technical visual, high resolution"

    if re.search(r"\b(?:software|platform|cloud|ai|machine learning|data science|backend|frontend|devops|security|network|cybersecurity|code|api|database)\b", normalized, flags=re.IGNORECASE):
        return f"high quality editorial photography of {prompt}, modern technology, digital interface, realistic"

    if re.search(r"\b(?:marketing|business|startup|strategy|growth|sales|branding|ecommerce|digital marketing)\b", normalized, flags=re.IGNORECASE):
        return f"high quality editorial photography of {prompt}, business strategy, modern workplace, professional"

    if re.search(r"\b(?:healthcare|medical|hospital|doctor|patient|biotech|wellness)\b", normalized, flags=re.IGNORECASE):
        return f"high quality editorial photography of {prompt}, healthcare, medical professionals, realistic"

    if re.search(r"\b(?:education|learning|students|school|training|classroom|course)\b", normalized, flags=re.IGNORECASE):
        return f"high quality editorial photography of {prompt}, education, classroom, modern learning"

    return f"{prompt}, high quality editorial photography, realistic, clear composition"
